...
const AdminDashboard = () => {
    ...
    const fetchStats = async () => {
        try {
            ...
            // Calculate Stats (safeguard provided)
            const totalInventoryValue = Array.isArray(invList)
                ? invList.reduce((acc, part) => acc + (parseFloat(part.price || 0) * (part.quantity || 0)), 0)
                : 0;

            const active = Array.isArray(jobsList)
                ? jobsList.filter(j => j.status === 'IN_PROGRESS' || j.status === 'DIAGNOSED').length
                : 0;

            const completed = Array.isArray(jobsList)
                ? jobsList.filter(j => j.status === 'COMPLETED').length
                : 0;

            const pending = Array.isArray(jobsList)
                ? jobsList.filter(j => j.status === 'PENDING').length
                : 0;

            const lowStock = Array.isArray(invList)
                ? invList.filter(part => part.quantity < 5)
                : [];
